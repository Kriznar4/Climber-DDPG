...
upper_bound = env.action_bound[1]
lower_bound = env.action_bound[0]
...
def get_actor():
    ...
    # middle_init = tf.random_uniform_initializer(minval=-0.003, maxval=0.003)
...
def policy(state, noise_object, step):
    ...
    # sampled_actions = np.array([a + noise_object() for a in sampled_actions])*0.01
    print(sampled_actions)
    sampled_actions = (sampled_actions.numpy()*0.6 + noise*0.01)*0.1
    if step <= 20:
...
for ep in range(total_episodes):
    ...
    for i in range(total_steps):
        ...
        action = policy(tf_prev_state, ou_noise, i)
        ...
        # if buffer.buffer_counter >= buffer.buffer_capacity:
        buffer.learn()
    ...
    print("Episode * {} * | Avg Reward is ==> {} | Total Reward is ==> {} | {}".format(ep, avg_reward, episodic_reward, done))
    with open('history.txt', 'a') as f:
        f.write("Episode * {} * | Avg Reward is ==> {} | Total Reward is ==> {} | {} \n".format(ep, avg_reward, episodic_reward, done))
